  ...
  #Read Files
    ...
    #Following are four ways to read in files from the readr library
      ...
      read_delim("path to file.txt", delim = , col_names = , col_types = ) #reads a table such as a text file as a dataframe 
                                                                           #delim = what is the separating factor 
                                                                                    #(comma, forward slash, tab, etc.)
    
      read_csv("path to file.csv", col_names = , col_types = ) #reads csv file where a common is the delimiter and 
                                                               #header is default TRUE
    
      read_csv2("path to file.csv", col_names = , col_types = ) #reads csv file where a semicolon is the delimiter and
                                                                #header is default TRUE
    
      read_tsv("path to file.txt", col_names = , col_types = )  #reads file where a tab is the delimiter and header is
                                                                #default TRUE
    
    library(data.table)           #created for faster development including but not limited to read, write, aggregations
    ...
    library(readxl)  #library used for reading in excel files
      ...
      read_excel("path to excel file", col_names = , col_types = , skip = ) #col_names is default to TRUE can be set with a c()
                                                     #col_types is default to be asessed by R but can also be set
                                                            #c("text", "numeric", "date", "blank"), "blank" will skip the col
                                                                    #skip is set to the number of row to skip before importing
                                                                    #if this is done then header row no longer exists
