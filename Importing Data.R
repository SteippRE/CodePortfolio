#Importing Data
  ...
  #Read Files
    ...
    #Following are four ways to read in files from the readr library
      ...
      read_csv2("path to file.csv", col_names = , col_types = ) #reads csv file where a semicolon is the delimiter 
                                                                #and header is default TRUE
    
      read_tsv("path to file.txt", col_names = , col_types = )  #reads file where a tab is the delimiter and hearder
                                                                #is default TRUE
    ...
    library(readxl)  #library used for reading in excel files
      ...
      read_excel("path to excel file")    #reads in the first sheet in the excel file, can specify which sheet to read in
          ...
          read_excel("path to excel file", sheet = ) #sheet can either be the num. of sheet index or the name of the sheet
          ...
          read_excel("path to excel file", col_names = , col_types = , skip = ) #col_names is default to TRUE can be set ith a c()
                                                                           ...
                                                                           #skip is set to the num. of rows to skip before importing 
